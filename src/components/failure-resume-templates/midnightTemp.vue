...
<template>
  <div class="resume-template-wrapper midnight-template">
    <div class="header">
      <div class="title">Failure Resume</div>
      <p class="text-description text-left fs-14">{{ displayName }} <span>•</span> {{ userName }}</p>
    </div>
    <div class="inner-wrapper">
      ...
      <template v-if="data.length > 0">
        <div class="hr mt-3 mb-5" />
        <div class="section-title">Major Failures</div>
        <ul class="selected-items">
          <li v-for="(item, index) in data" :key="item.id">
            <h6 class="font-weight-semibold fs-14 text-white">
              <span>{{ index +1 }}.</span>
              {{ item.title }}
            </h6>
      ...
      <template v-if="grow.length > 0">
        ...
        <ul class="grow-items mt-4">
          <li v-for="(item, index) in grow" :key="index" class="mb-2">
            <div class="fs-12 text-white font-weight-semibold">{{ item.title }}</div>
      ...
      <template v-if="keyLessonsTemplate(data).length > 0">
        <div class="hr my-5" />
        <div class="section-title">Key Lessons</div>
