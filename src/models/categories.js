export const categories = [
  { id: 'business', label: 'Business / Career' },
  { id: 'relationships', label: 'Relationships' },
  { id: 'finance', label: 'Finance' },
  { id: 'health', label: 'Health / Wellness' },
  { id: 'education', label: 'Education / Learning' },
  { id: 'parenting', label: 'Parenting / Family' },
  { id: 'personal', label: 'Personal Goals' },
  { id: 'life', label: 'Everyday Life' },
  { id: 'other', label: 'Other' },
]

export const recoveryTimeOptions = [
  { title: '1 week', value: '1-week', days: 7 },
  { title: '1 month', value: '1-month', days: 30 },
  { title: '6 months', value: '6-months', days: 180 },
  { title: '1 year', value: '1-year', days: 365 },
  { title: '> 1 year', value: 'over-1-year', days: 366 },
]
export const emotionTags = [
  { label: 'Painful', emoji: '🥺', value: 'painful' },
  { label: 'Embarrassing', emoji: '😅', value: 'embarrassing' },
  { label: 'Learning', emoji: '🧠', value: 'learning' },
  { label: 'Frustrating', emoji: '😤', value: 'frustrating' },
  { label: 'Disappointing', emoji: '😔', value: 'disappointing' },
  { label: 'Relieved', emoji: '😌', value: 'relieved' },
  { label: 'Angry', emoji: '😠', value: 'angry' },
  { label: 'Sad', emoji: '😢', value: 'sad' },
  { label: 'Anxious', emoji: '😰', value: 'anxious' },
  { label: 'Regretful', emoji: '😞', value: 'regretful' },
  { label: 'Overwhelmed', emoji: '😵', value: 'overwhelmed' },
  { label: 'Shocked', emoji: '😱', value: 'shocked' },
  { label: 'Hopeful', emoji: '🙂', value: 'hopeful' },
  { label: 'Grateful', emoji: '🙏', value: 'grateful' },
  { label: 'Confused', emoji: '😕', value: 'confused' },
]
export const visibilityList = [
  {
    label: 'Public',
    value: 1,
  },
  {
    label: 'Followers Only',
    value: 2,
  },
  {
    label: 'Private',
    value: 3,
  },
]

export const costRange = [
  {
    label: 'Free',
    value: 'free',
  },
  {
    label: '<$100',
    value: '<100',
  },
  {
    label: '$100 - $1k',
    value: '100-1000',
  },
  {
    label: '$1k - $5k',
    value: '1000-5000',
  },
  {
    label: '$5k+',
    value: '>5000',
  },
]
